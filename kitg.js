...
function autoTrade() {
        ...
        if ((gamePage.resPool.get('titanium').value < Math.min(gamePage.resPool.get('paragon').value < 1000 ? gamePage.resPool.get('paragon').value : 200000 || gamePage.resPool.get('blueprint').value < 100) && gamePage.diplomacy.get('zebras').unlocked && gamePage.resPool.get('gold').value < gamePage.resPool.get('gold').maxValue * 0.95) {
...
function autoCraft2() {
        ...
        if (gamePage.science.get("construction").researched && gamePage.tabs[3].visible ) {
            let  resourcesAll = [
                ["beam", [["wood",175]],Math.min(gamePage.resPool.get("wood").value/175+1,50000)],
                ["slab", [["minerals",250]], Math.min(gamePage.resPool.get("minerals").value/250+1,50000)],
...
function autoResearch() {
    if (gamePage.tabs[2].visible) {
        ...
        var btn = gamePage.tabs[2].buttons.filter(res => res.model.metadata.unlocked && res.model.metadata.name != 'retorting');
        for (var i = 0; i < btn.length; i++) {
...
function autoAssign() {
        var resourcesAssign = [
            ...
            ["faith", "priest",gamePage.tabs[5].rUpgradeButtons.filter(res => res.model.resourceIsLimited == false && (!(res.model.name.includes('(complete)'))) && (!(res.model.name.includes('(Transcend)')))).length  == 0 ? 50+gamePage.religion.tclevel : 1,5],
