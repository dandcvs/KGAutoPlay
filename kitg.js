...
var resourcesAll = [
    ...
    ["tanker", [["ship",200],["kerosene",200],["alloy",1250],["blueprint",5]],0],
    ["kerosene", [["oil",7500]],0],
...
// Trade automatically
function autoTrade() {
        if  (gamePage.resPool.get('uranium').value <  Math.min(gamePage.resPool.get('paragon').value,100) && gamePage.diplomacy.get('dragons').unlocked) {
            gamePage.diplomacy.tradeAll(game.diplomacy.get("dragons"), 1);
        }
        if (gamePage.resPool.get('titanium').value < Math.min(gamePage.resPool.get('paragon').value,7500) && gamePage.diplomacy.get('zebras').unlocked) {
            gamePage.diplomacy.tradeAll(game.diplomacy.get("zebras"), 1);
        }
        if((gamePage.religion.getRU('solarRevolution').val == 1) || (gamePage.ironWill)){
...
function autoAssign() {
        var resourcesAssign = [
            ...
            ["wood", "woodcutter",1,2],
        	["minerals", "miner",1,2],
            ["science", "scholar",gamePage.resPool.get("science").value == gamePage.resPool.get("science").maxValue1 ? 100 : 1,5],
        	["manpower", "hunter",1,5],
...
function Service(){
    ...
    resourcesAll = [
        ...
        ["scaffold", [["beam",50]],1000],
        ["ship", [["scaffold",100],["plate",150],["starchart",25]],100],
        ["tanker", [["ship",200],["kerosene",gamePage.resPool.get('oil').maxValue * 0.5],["alloy",1250],["blueprint",5]],0],
        ["kerosene", [["oil",7500]],0],
        ["parchment", [["furs",175]],0],
        ["manuscript", [["parchment",25],["culture",400]],gamePage.ironWill ? 0 : 35],
        ["compedium", [["manuscript",50],["science",10000]],gamePage.ironWill ? f(gamePage.resPool.get('science').value)*2 : 0],
