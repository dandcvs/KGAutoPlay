                    ...
                    for (i = 0 ;i < spBuild.length; i++) {
                        if (spBuild[i].model.metadata.unlocked) {
                            if (!switches['CollectResBReset'] || spBuild[i].model.prices.filter(res => res.name == 'relic' || res.name == 'timeCrystal' || res.name == 'void').length == 0) {
                                ...
                                else if (gamePage.bld.getBuildingExt('chronosphere').meta.val < 10 && gamePage.resPool.get("timeCrystal").value >= Chronosphere10SummPrices()[1].val && spBuild[i].model.prices.filter(res => res.name == 'unobtainium' || res.name == 'eludium').length > 0){
                                    {}
                                }
                                else if (gamePage.ironWill){
        ...
        if (gamePage.science.get("construction").researched && gamePage.tabs[3].visible ) {
            ...
            if (!gamePage.ironWill && (cntcrafts == 0 || cntcrafts >= 200 || (Object.keys(craftPriority[0]).length > 0 && craftPriority[2] != gamePage.bld.getBuildingExt(craftPriority[0]).meta.val))) {
                var Priority_blds = {
                    ...
                    "warehouse" : 0.01,
                    "harbor" : gamePage.bld.getBuildingExt('harbor').meta.val > 100 ? 1 : 0.001,
                    "smelter" : 4,
...
function autoAssign() {
        var resourcesAssign = [
       		["catnip", "farmer",gamePage.resPool.get("beam").value > gamePage.resPool.get("catnip").maxValue ? gamePage.resPool.get("beam").value / gamePage.resPool.get("catnip").maxValue + 9 :9, gamePage.resPool.get('paragon').value < 100 ? 0.1 : 1],
            ...
            ["faith", "priest",gamePage.tabs[5].rUpgradeButtons.filter(res => res.model.resourceIsLimited == false && (!(res.model.name.includes('(complete)'))) && (!(res.model.name.includes('(Transcend)')))).length  == 0 ?  gamePage.religion.tclevel+1 : gamePage.resPool.get("faith").value/gamePage.resPool.get("faith").maxValue * 10 + 1 , 2],
...
function LabelMsg(){
    ...
    if (gamePage.bld.getBuildingExt('chronosphere').meta.unlocked && gamePage.bld.getBuildingExt('chronosphere').meta.val < 10 && gamePage.resPool.get("timeCrystal").value > 0){
