        ...
        if (gamePage.science.get("construction").researched && gamePage.tabs[3].visible ) {
            ...
            for (var i = 0; i < resourcesAllF.length; i++) {
                ...
                if (gamePage.workshop.getCraft(resourcesAllF[i][0]).unlocked) {
                     flag = true;
                     cnt = 0;
                     if (curResTarget.value <= Math.min(resourcesAllF[i][2] , gamePage.resPool.get('paragon').value) && gamePage.resPool.get(resourcesAllF[i][1][0][0]).value/gamePage.resPool.get(resourcesAllF[i][0]).value > gamePage.resPool.get(resourcesAllF[i][0]).value) {
