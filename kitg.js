...
// Trade automatically
function autoTrade() {
        if  (gamePage.resPool.get('titanium').value > 1000 && gamePage.resPool.get('uranium').value <  Math.min(gamePage.resPool.get('paragon').value,25) && gamePage.diplomacy.get('dragons').unlocked && gamePage.resPool.get('gold').value < gamePage.resPool.get('gold').maxValue * 0.95) {
...
function autoCraft2() {
        ...
        if (gamePage.science.get("construction").researched && gamePage.tabs[3].visible ) {
            let  resourcesAll = [
                ...
                ["gear", [["steel",15]],25],
                ["alloy", [["steel",75],["titanium",10]],Math.min(Math.max(gamePage.resPool.get("steel").value/75*gamePage.getCraftRatio()+1,gamePage.resPool.get("titanium").value/10*gamePage.getCraftRatio()+1,100),1000)],
